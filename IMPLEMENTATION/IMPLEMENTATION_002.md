...
- `/analyze/<code>/financial` 엔드포인트 추가
  ...
  → {   // 아래는 형식 설명용 예시 값 (실제 값 아님)
      "code": "005930",
      "as_of": "2026-06-24",   // 재무 지표 기준 거래일 (PYKRX)
      "per": 8.5,
      "pbr": 1.2,
      "roe": 7.0,
      "eps": 3500,
  ...
  ```
- **대시보드 UI** (HTML 템플릿, 후속 추가):
  - **기준일 표시**: 판정 결과 아래에 `기준일: {as_of}` (재무 지표 기준 거래일).
  - **판정 근거 tooltip**: "판정 결과" 옆 `?`(`tooltip-icon`) hover 시 판정 근거 표시. 응답 데이터로 동적 생성 — 판정 규칙 + `PER {값} vs 업계 {평균}` · `PBR {값} vs 업계 {평균}` 비교 + 종합 판정.
