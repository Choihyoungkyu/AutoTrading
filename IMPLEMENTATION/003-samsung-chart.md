...
> **갱신(이슈 004 작업 중)**: yfinance가 국내 종목(`005930.KS`)을 조회하지 못해 데이터 소스를
> **PYKRX로 전환**했다(커밋 `d26bc02`). 지표 계산·신호 로직은 동일하며, 수집만
> `krx.get_ohlcv(code, start, end)`로 약 1년 범위를 조회한다. 아래 "데이터 수집" 항목의
> yfinance 설명은 과거 기록이다.

## 개요

`ta` 라이브러리로 삼성전자 1년치 OHLCV의 기술적 지표를 계산한 뒤 매수/매도/중립 신호를 판단하는 API를 구현했다. (초기엔 yfinance로 수집했으나 이후 PYKRX로 전환 — 위 갱신 참고.)
...
> 응답에 `as_of`(분석에 사용한 최신 봉 날짜)가 포함되며, 대시보드 차트 분석 섹션의 매매 신호 아래에 `기준일: {as_of}`로 표시된다.
